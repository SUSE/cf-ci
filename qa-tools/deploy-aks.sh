...
if [[ "$OSTYPE" == "darwin"* ]]; then
  export KUBECONFIG="/private"$(mktemp -d)/config
else
...
docker exec -i aks-deploy kubectl create -f persistent-sc.yaml
docker exec -i aks-deploy kubectl apply -f cluster-admin.yaml
