...
# Wait for CF release
# TBD: Apply proper wait using
# https://github.com/SUSE/kubecf/blob/master/.gitlab/pipelines/stages/deploy/wait_kubecf.sh
# because CF pods are being generated by cfo and you may get a false possitive
sleep 600
kubectl wait --timeout=30m --namespace "${CF_NAMESPACE}" --for=condition=ready pod --all
helm list
kubectl get ns
kubectl get pods --namespace "${CF_NAMESPACE}"
