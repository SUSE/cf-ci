...
image=$(awk '/image/ { print $2 }' < scf-config/kube/cf/bosh-task/smoke-tests.yml)
sed -i "s/cf-dev\.io/${DOMAIN}/g" scf-config/kube/cf/bosh-task/smoke-tests.yml
jsonify() { ruby -r yaml -r json -e 'YAML.load_stream(File.read "'"$1"'").each { |yaml| puts yaml.to_json}'; }
kubectl run -n cf --attach --restart=Never --image ${image} --overrides="$(jsonify scf-config/kube/cf/bosh-task/smoke-tests.yml)" smoke-tests
