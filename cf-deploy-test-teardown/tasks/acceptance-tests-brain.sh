...
image=$(awk '/image/ { print $2 }' < scf-alpha/kube/cf/bosh-task/acceptance-tests-brain.yml)
sed -i "s/cf-dev\.io/${DOMAIN}/g" scf-alpha/kube/cf/bosh-task/acceptance-tests-brain.yml
jsonify() { ruby -r yaml -r json -e 'YAML.load_stream(File.read "'"$1"'").each { |yaml| puts yaml.to_json}'; }
kubectl run -n cf --attach --restart=Never --image ${image} --overrides="$(jsonify scf-alpha/kube/cf/bosh-task/acceptance-tests-brain.yml)" acceptance-tests-brain